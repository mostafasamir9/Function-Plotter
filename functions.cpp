...
QMessageBox msgBox; //initiate QMessagebox Object
QString errormsg = msg;
msgBox.setText(errormsg); // set the text of the message box
msgBox.exec(); // launch message box
...
void parseEquation(char* expression, QString equation , QVector<double> &x_arr, QVector<double> &y_arr, double x_min_d ,double x_max_d){
    ...
    if (expr) {
        ...
        for (double i=x_min_d; i<x_max_d+.1/PRECISION; i+=1.0/PRECISION)
        {
            ...
            if(x < .1/PRECISION && x > -.1/PRECISION) x = 0;
